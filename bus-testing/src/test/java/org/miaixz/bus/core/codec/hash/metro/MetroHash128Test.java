...
public class MetroHash128Test {
    ...
    static final byte[] ENDIAN =
            ByteKit.toBytes("012345678901234567890123456789012345678901234567890123456789012");

    static String h128(final String input) {
        final MetroHash128 mh = MetroHash128.of(0).apply(ByteBuffer.wrap(ByteKit.toBytes(input)));
        final No128 hash = mh.get();
        return hex(hash.getMostSigBits()) + hex(hash.getLeastSigBits());
    }

    private static String hex(final long value) {
        return HexKit.toHex(value).toUpperCase();
    }

    private static String hex(final byte[] bytes) {
        return HexKit.encodeString(bytes).toUpperCase();
    }
