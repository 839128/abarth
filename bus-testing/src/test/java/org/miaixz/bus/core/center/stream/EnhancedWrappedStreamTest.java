...
/**
 * {@link EnhancedWrappedStream}、{@link TerminableWrappedStream}、{@link TransformableWrappedStream}的测试用例。
 * 此用例用于保证通过{@link EnhancedWrappedStream}获得的默认方法，在子类不重写的情况下能够按照预期效果生效
 */
public class EnhancedWrappedStreamTest {
    ...
    public void testToZip() {
        ...
        final List<String> list = asList("miaixz", "bus", "sweet");
    ...
    public void testFlatTree() {
        final Tree root = new Tree(1, ListKit.of(new Tree(2, ListKit.of(new Tree(3, Collections.emptyList())))));
    ...
    public void testMapNonNull() {
        Assertions.assertEquals(
                ListKit.of("3"), wrap(null, 2, 3, 4).mapNonNull(t -> ((t & 1) == 0) ? null : String.valueOf(t)).toList()
        );
    }
    ...
    public void testZip() {
        ...
        Assertions.assertEquals(Arrays.asList("1.miaixz", "2.bus", "3.sweet"), zip);
    ...
    public void testListSplit() {
        ...
        Assertions.assertEquals(ListKit.partition(list, 2), lists);
    ...
    private static class Wrapper<T> extends EnhancedWrappedStream<T, Wrapper<T>> {
