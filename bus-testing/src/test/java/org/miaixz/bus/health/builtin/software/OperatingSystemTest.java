...
class OperatingSystemTest {
    ...
    private OperatingSystem os = new Platform().getOperatingSystem();
    ...
    void testProcessStats() {
        assertThat("Current running process state shouldn't be INVALID", proc.getState(), is(not(OSProcess.State.INVALID)));
    ...
    void testGetChildAndDependentProcesses() {
        ...
        if (manyChildMap.size() > 9) {
            ...
            for (Integer i : manyChildMap.keySet()) {
                ...
                List<OSProcess> children = os.getChildProcesses(i, OperatingSystem.ProcessFiltering.VALID_PROCESS,
                        OperatingSystem.ProcessSorting.CPU_DESC, Integer.MAX_VALUE);
                List<OSProcess> descendants = os.getDescendantProcesses(i, OperatingSystem.ProcessFiltering.VALID_PROCESS,
                        OperatingSystem.ProcessSorting.CPU_DESC, Integer.MAX_VALUE);
                if (children.size() > 0) {
                    matchedChild++;
                }
