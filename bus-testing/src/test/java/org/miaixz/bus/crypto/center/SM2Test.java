...
public class SM2Test {
    ...
    public void KeyPairOIDTest() {
        ...
        final KeyPair pair = Keeper.generateKeyPair("SM2");
        Assertions.assertTrue(HexKit.encodeString(pair.getPrivate().getEncoded()).toUpperCase().contains(OID));
        Assertions.assertTrue(HexKit.encodeString(pair.getPublic().getEncoded()).toUpperCase().contains(OID));
    }
    ...
    public void sm2CustomKeyTest() {
        ...
        final SM2 sm2 = Builder.sm2(privateKey, publicKey);
        sm2.setMode(SM2Engine.Mode.C1C2C3);

        // 公钥加密，私钥解密
        final byte[] encrypt = sm2.encrypt(ByteKit.toBytes("我是一段测试aaaa", Charset.UTF_8), KeyType.PublicKey);
    ...
    public void sm2Base64Test() {
        ...
        final String decryptStr = StringKit.toString(sm2.decrypt(encryptStr, KeyType.PrivateKey));
        ...
        final String decryptStr2 = StringKit.toString(sm2.decrypt(encryptStr, KeyType.PrivateKey));
    ...
    public void sm2VerifyTest() {
        ...
        final boolean verify = sm2.verify(dataBytes, HexKit.decode(signHex));
    ...
    public void sm2SignAndVerifyHexTest() {
        ...
        final String sign = sm2.signHex(HexKit.encodeString(content));
        final boolean verify = sm2.verifyHex(HexKit.encodeString(content), sign);
        Assertions.assertTrue(verify);
    }
    ...
    public void sm2SignAndVerifyUseKeyTest2() {
        ...
        final SM2 sm2 = new SM2(
                HexKit.encodeString(pair.getPrivate().getEncoded()),
                HexKit.encodeString(pair.getPublic().getEncoded())
        );
    ...
    public void sm2PublicKeyEncodeDecodeTest() {
        ...
        final byte[] data = Keeper.encodeECPublicKey(publicKey);
        final String encodeHex = HexKit.encodeString(data);
        final String encodeB64 = Base64.encode(data);
        final PublicKey Hexdecode = Keeper.decodeECPoint(encodeHex, Builder.SM2_CURVE_NAME);
        final PublicKey B64decode = Keeper.decodeECPoint(encodeB64, Builder.SM2_CURVE_NAME);
        Assertions.assertEquals(HexKit.encodeString(publicKey.getEncoded()), HexKit.encodeString(Hexdecode.getEncoded()));
        Assertions.assertEquals(HexKit.encodeString(publicKey.getEncoded()), HexKit.encodeString(B64decode.getEncoded()));
    }
    ...
    public void sm2PlainWithPointTest2() {
        ...
        final String decryptStr = sm2.decryptString(encryptHex, KeyType.PrivateKey);
    ...
    public void getPublicKeyByPrivateKeyTest() {
        ...
        final PrivateKey privateKey = Keeper.generatePrivateKey("sm2", new OpenSSHPrivateKeySpec(Builder.decode(priKey)));
        final ECPrivateKeyParameters privateKeyParameters = Keeper.toPrivateParams(privateKey);

        final SM2 sm2 = new SM2(privateKeyParameters, Keeper.getPublicParams(privateKeyParameters));
    ...
    public void readPublicKeyTest() {
        ...
        final SM2 sm2 = Builder.sm2(priKey, pubKey);
