...
public class RSATest {
    ...
    public void generateKeyPairTest() {
        final KeyPair pair = Keeper.generateKeyPair("RSA");
    ...
    public void rsaCustomKeyTest() {
        ...
        final RSA rsa = Builder.rsa(privateKey, publicKey);

        // 公钥加密，私钥解密
        final byte[] encrypt = rsa.encrypt(ByteKit.toBytes("我是一段测试aaaa", Charset.UTF_8), KeyType.PublicKey);
        ...
        // 私钥加密，公钥解密
        final byte[] encrypt2 = rsa.encrypt(ByteKit.toBytes("我是一段测试aaaa", Charset.UTF_8), KeyType.PrivateKey);
        final byte[] decrypt2 = rsa.decrypt(encrypt2, KeyType.PublicKey);
        Assertions.assertEquals("我是一段测试aaaa", StringKit.toString(decrypt2, Charset.UTF_8));
    }
    ...
    public void rsaECBTest() {
        final RSA rsa = new RSA(Algorithm.RSA_ECB.getValue());
    ...
    public void rsaNoneTest() {
        final RSA rsa = new RSA(Algorithm.RSA_NONE.getValue());
    ...
    public void rsaBase64Test() {
        ...
        final String decryptStr = StringKit.toString(rsa.decrypt(encryptStr, KeyType.PrivateKey));
        Assertions.assertEquals(text.toString(), decryptStr);

        // 私钥加密，公钥解密
        final String encrypt2 = rsa.encryptBase64(text.toString(), KeyType.PrivateKey);
        final String decrypt2 = StringKit.toString(rsa.decrypt(encrypt2, KeyType.PublicKey));
    ...
    public void rsaDecodeTest() {
        ...
        final byte[] aByte = HexKit.decode(a);
        final byte[] decrypt = rsa.decrypt(aByte, KeyType.PrivateKey);

        Assertions.assertEquals("虎头闯杭州,多抬头看天,切勿只管种地", StringKit.toString(decrypt, Charset.UTF_8));
    }
    ...
    public void rsaTest2() throws Exception {
        ...
        final PublicKey publicKey = Keeper.generateRSAPublicKey(keyBytes);

        final byte[] data = RandomKit.randomString("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", 16).getBytes();
        //长度不满足128补0
        final byte[] finalData = ArrayKit.resize(data, 128);

        // jdk原生加密
        final Cipher cipher = Cipher.getInstance("RSA/ECB/NoPadding");
        cipher.init(Cipher.ENCRYPT_MODE, publicKey);
        final String result1 = HexKit.encodeString(cipher.doFinal(finalData));
