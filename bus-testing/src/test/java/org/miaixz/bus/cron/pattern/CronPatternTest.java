...
public class CronPatternTest {
    ...
    public void matchAllTest() {
        ...
        assertMatch(pattern, DateKit.formatNow());
    }
    ...
    public void matchAllTest2() {
        ...
        // 测试时，秒归零，则任意时间匹配
        assertMatch(pattern, DateKit.beginOfMinute(DateKit.now()).toString());
    }
    ...
    public void CronPatternTest2() {
        ...
        Assertions.assertTrue(pattern.match(DateKit.parse("2018-10-09 12:00:00").getTime(), false));
        Assertions.assertTrue(pattern.match(DateKit.parse("2018-10-09 12:30:00").getTime(), false));

        pattern = CronPattern.of("32 * * * *");
        Assertions.assertTrue(pattern.match(DateKit.parse("2018-10-09 12:32:00").getTime(), false));
    }
    ...
    private void assertMatch(final CronPattern pattern, final String date) {
        Assertions.assertTrue(pattern.match(DateKit.parse(date).toCalendar(), false));
        Assertions.assertTrue(pattern.match(DateKit.parse(date).toCalendar(), true));
    }

    @Test
    public void matchedDatesTest() {
        //测试每30秒执行
        final List<Date> matchedDates = CronPattern.matchedDates("0/30 * 8-18 * * ?", DateKit.parse("2018-10-15 14:33:22"), 5);
    ...
    public void matchedDatesTest2() {
        //测试每小时执行
        final List<Date> matchedDates = CronPattern.matchedDates("0 0 */1 * * *", DateKit.parse("2018-10-15 14:33:22"), 5);
        Assertions.assertEquals(5, matchedDates.size());
    ...
    public void matchedDatesTest3() {
        //测试最后一天
        final List<Date> matchedDates = CronPattern.matchedDates("0 0 */1 L * *", DateKit.parse("2018-10-30 23:33:22"), 5);
        Assertions.assertEquals(5, matchedDates.size());
