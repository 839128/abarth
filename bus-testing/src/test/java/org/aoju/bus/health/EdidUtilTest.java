...
public class EdidUtilTest {
    ...
    private final static String EDID_STR2 = EDID_HEADER + EDID_MANUFID + EDID_PRODCODE + EDID_SERIAL + EDID_WKYR
            ...
            + EDID_CHKSUM;
    private final static byte[] EDID = Builder.hexStringToByteArray(EDID_STR);

    @Test
    public void testGetEdidAttrs() {
        assertEquals("A", Builder.getManufacturerID(EDID));
        assertEquals("9227", Builder.getProductID(EDID));
        assertEquals("162C0C25", Builder.getSerialNo(EDID));
        assertEquals((byte) 44, Builder.getWeek(EDID));
        assertEquals(2012, Builder.getYear(EDID));
        assertEquals("1.4", Builder.getVersion(EDID));
        assertTrue(Builder.isDigital(EDID));
        assertEquals(60, Builder.getHcm(EDID));
        assertEquals(34, Builder.getVcm(EDID));
    }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            switch (i) {
                ...
                case 2:
                    ...
                    assertEquals("C02JM2PFF2GC", Builder.getDescriptorText(descs[i]));
                    assertEquals(EDID_DESC3, Builder.byteArrayToHexString(descs[i]));
                    break;
                case 3:
                    assertEquals(0xFC, type);
                    assertEquals("Thunderbolt", Builder.getDescriptorText(descs[i]));
                    break;
                default:
            }
        }
    ...
    public void testToString() {
        String[] toString = Builder.toString(EDID).split("\\n");
        assertEquals(6, toString.length);
        toString = Builder.toString(Builder.hexStringToByteArray(EDID_STR2)).split("\\n");
        assertEquals(6, toString.length);
    }
